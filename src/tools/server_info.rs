...
impl ToolEngine {
    pub fn server_info(&self) -> Result<Value, ToolFailure> {
        ...
        set_nested(
            ...
            "save_dir_root",
            Value::String(format!(
                "所有输出强制落在 TWGGA_SAVE_DIR_ROOT={} 之下；传 root 之外路径会被拒",
...
fn input_validation_description(paths: &crate::config::AppPaths) -> Result<String, ToolFailure> {
    let relative_rule = if let Some(root) = &paths.input_root {
        format!(
            "相对输入路径相对于 TWGGA_INPUT_ROOT={} 解析，所有输入均受该 capability root 限制",
            path_text(root, "input root")?
        )
    } else {
        format!(
            "未设置 TWGGA_INPUT_ROOT 时，相对输入路径相对于 server 启动时捕获的 cwd={} 解析",
