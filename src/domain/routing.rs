...
pub fn resolve_model(
    ...
    let model = requested.unwrap_or(default_model).to_owned();
...
/// 本站没有独立的高质量线路，恒为 false。
///
/// 调用方据此走标准线路的并发与重试策略；那两项本来是为付费队列的限流准备的。
pub fn is_quality_model(_model: &str) -> bool {
    false
}
