...
pub fn merge_config(existing: &str, launch: &ClientLaunchSpec) -> Result<String, InstallError> {
    ...
    let server = twgga_server(&mut document)?;
    server.insert(
        "command".into(),
...
pub fn parse_config_launch(input: &str) -> Result<Option<ClientLaunchSpec>, InstallError> {
    ...
    let Some(server) = document
        ...
        .and_then(|servers| servers.get("twgga-image"))
        .and_then(Value::as_object)
    else {
        return Ok(None);
    };
    let command = server
        ...
        .ok_or_else(|| InstallError::InvalidClaude("twgga-image.command 必须是 string".into()))?;
    let args = server
        .get("args")
        .and_then(Value::as_array)
        .ok_or_else(|| InstallError::InvalidClaude("twgga-image.args 必须是 array".into()))?
        .iter()
        .map(|value| {
            value.as_str().map(OsString::from).ok_or_else(|| {
                InstallError::InvalidClaude("twgga-image.args 只能包含 string".into())
            })
        })
...
pub fn reset_config(existing: &str) -> Result<Option<String>, InstallError> {
    ...
    let changed = document
        ...
        .and_then(|servers| servers.remove("twgga-image"))
        .is_some();
    if !changed {
        return Ok(None);
    }
...
fn twgga_server(document: &mut Value) -> Result<&mut Map<String, Value>, InstallError> {
    let root = document
        .as_object_mut()
    ...
    servers
        ...
        .or_insert_with(|| Value::Object(Map::new()))
        .as_object_mut()
        .ok_or_else(|| InstallError::InvalidClaude("mcpServers.twgga-image 必须是 object".into()))
}
...
fn environment_object(
    ...
    server
        ...
        .as_object_mut()
        .ok_or_else(|| {
            InstallError::InvalidClaude("mcpServers.twgga-image.env 必须是 object".into())
        })
}
        ...
        .map(|(name, value)| {
            let value_text = value.as_str().ok_or_else(|| {
                InstallError::InvalidClaude(format!(
                    "mcpServers.twgga-image.env.{name} 必须是 string"
                ))
            })?;
...
pub(crate) fn verify_round_trip(
    ...
    let actual = parse_config_launch(rendered)?
        .ok_or_else(|| InstallError::ClaudeRoundTrip("缺少 mcpServers.twgga-image".into()))?;
