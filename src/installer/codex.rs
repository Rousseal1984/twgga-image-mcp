...
pub fn merge_config(existing: &str, launch: &ClientLaunchSpec) -> Result<String, InstallError> {
    ...
    let server = twgga_server_table(&mut document)?;
...
pub fn parse_config_launch(input: &str) -> Result<Option<ClientLaunchSpec>, InstallError> {
    ...
    let command = server
        ...
        .ok_or_else(|| InstallError::InvalidCodex("twgga-image.command 必须是 string".into()))?;
    let args = server
        .get("args")
        .and_then(Item::as_value)
...
fn twgga_server_table(document: &mut DocumentMut) -> Result<&mut Table, InstallError> {
    ...
    if servers.get("twgga-image").is_none() {
        servers.insert("twgga-image", Item::Table(Table::new()));
    }
    servers
        .get_mut("twgga-image")
        .and_then(Item::as_table_mut)
        .ok_or_else(|| InstallError::InvalidCodex("mcp_servers.twgga-image 必须是 table".into()))
}
        ...
        .map(|(name, item)| {
            let value_text = item.as_value().and_then(Value::as_str).ok_or_else(|| {
                InstallError::InvalidCodex(format!(
                    "mcp_servers.twgga-image.env.{name} 必须是 string"
                ))
            })?;
...
pub(crate) fn verify_round_trip(
    ...
    let actual = parse_config_launch(rendered)?
        .ok_or_else(|| InstallError::CodexRoundTrip("缺少 mcp_servers.twgga-image".into()))?;
