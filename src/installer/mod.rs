...
const DEFAULT_BASE_URL: &str = "https://twgga.work";
...
pub fn install(options: InstallOptions) -> Result<(), InstallError> {
    ...
    if let Some(save_dir) = &options.save_dir {
        ...
        environment.insert("TWGGA_SAVE_DIR_ROOT", value.clone());
        environment.insert("TWGGA_SAVE_DIR", value);
    } else if environment.get("TWGGA_SAVE_DIR_ROOT").is_none()
        && let Some(save_dir) = environment.get("TWGGA_SAVE_DIR").map(str::to_owned)
    {
        environment.insert("TWGGA_SAVE_DIR_ROOT", save_dir);
    }
    let base_url = validated_base_url(
        options
            .base_url
            .as_deref()
            .or_else(|| environment.get("TWGGA_BASEURL"))
            .unwrap_or(DEFAULT_BASE_URL),
    )?;
    environment.insert("TWGGA_BASEURL", base_url.clone());
...
pub fn reset(options: ResetOptions) -> Result<(), InstallError> {
    if !options.yes && !confirm("仅移除 twgga-image 配置并保留其他 MCP server，继续？")?
    {
        return Err(InstallError::Cancelled);
    }
    ...
    if !options.no_claude && claude::reset_config_file(&paths.claude_config)?.is_some() {
        write_status("已从 Claude JSON 移除 mcpServers.twgga-image".into())?;
    }
    if !options.no_codex && codex::reset_config_file(&paths.codex_config)?.is_some() {
        write_status("已从 Codex TOML 移除 mcp_servers.twgga-image".into())?;
    }
    Ok(())
}
...
pub fn doctor() -> Result<(), InstallError> {
    ...
    if configured == 0 {
        return Err(InstallError::Doctor(
            "Codex/Claude 均未配置 twgga-image".into(),
        ));
    }
...
fn install_environment(
    ...
    let mut environment = BTreeMap::from([
        (
            "TWGGA_SAVE_DIR".into(),
            path_text(&paths.default_save_dir, "TWGGA_SAVE_DIR")?,
        ),
        (
            "TWGGA_SAVE_DIR_ROOT".into(),
            path_text(&paths.save_root, "TWGGA_SAVE_DIR_ROOT")?,
        ),
    ]);
    if base_url != DEFAULT_BASE_URL {
        environment.insert("TWGGA_BASEURL".into(), base_url.into());
    }
    if let Some(input_root) = &paths.input_root {
        environment.insert(
            "TWGGA_INPUT_ROOT".into(),
            path_text(input_root, "TWGGA_INPUT_ROOT")?,
        );
    }
    for name in [
        "TWGGA_MODEL",
