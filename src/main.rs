...
#[derive(Debug, Parser)]
#[command(
    name = "twgga-image-mcp",
    version,
    about = "Native Twgga Image MCP server"
)]
struct Cli {
    #[command(subcommand)]
...
enum Command {
    ...
    /// Remove only the twgga-image MCP configuration.
    Reset {
        #[arg(long)]
        no_codex: bool,
        #[arg(long)]
...
async fn main() {
    if let Err(error) = run().await {
        ...
        let _ignored = writeln!(stderr, "twgga-image-mcp: {error}");
        std::process::exit(1);
    }
}
...
async fn serve() -> Result<(), Box<dyn Error>> {
    ...
    let server = TwggaServer::new(engine)?;
    let running = server
