...
pub enum PathError {
    ...
    #[error("TWGGA_INPUT_ROOT 必须存在且是目录: {0}")]
    InputRoot(String),
}
...
impl AppPaths {
    pub fn resolve(
        ...
        let raw_save_root = environment.get("TWGGA_SAVE_DIR_ROOT");
        let configured_root = match raw_save_root {
            Some(raw) => resolve_root(raw, &home, "TWGGA_SAVE_DIR_ROOT")?,
            None => home.join("Pictures/twgga-out"),
        };
        fs::create_dir_all(&configured_root).map_err(|error| PathError::CreateDirectory {
            context: "TWGGA_SAVE_DIR_ROOT",
            detail: error.to_string(),
        })?;
        let save_root =
        ...
        let default_save_dir = match environment.get("TWGGA_SAVE_DIR") {
            Some(raw) => resolve_under_root(raw, &home, &save_root, "TWGGA_SAVE_DIR")?,
            None => save_root.clone(),
        };
        let default_save_dir =
            create_directory_within(&save_root, &default_save_dir, "TWGGA_SAVE_DIR")?;

        let input_root = environment
            .get("TWGGA_INPUT_ROOT")
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(|raw| {
                let requested = resolve_root(raw, &home, "TWGGA_INPUT_ROOT")?;
        ...
        let cache_dir = home.join(".cache/twgga-image");
        ...
        Ok(Self {
            ...
            startup_cwd,
            executable,
            install_binary: data_local.join("twgga-image-mcp/bin").join(binary_name),
