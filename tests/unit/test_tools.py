...
import server
from twgga_image_mcp import http_client, routing, save
from twgga_image_mcp.save import ImageSaveError
...
def test_server_info_reports_only_image2_models():
    ...
    assert r["version"] == "0.4.1"
    assert r["available_models"] == ["gpt-image-2"]
...
def test_live_benchmark_requires_explicit_gate_even_when_key_exists(script):
    env = os.environ.copy()
    env["TWGGA_API_KEY"] = "sk-test-only"
    env.pop("TWGGA_RUN_LIVE_TESTS", None)
...
def test_save_extracted_payload_url_then_b64(monkeypatch, tmp_path):
    ...
    from twgga_image_mcp import config
