...
from twgga_image_mcp import config, http_client, io_safety, save
from twgga_image_mcp.save import ImageSaveError, _assert_download_url_safe
...
def test_input_root_jail(monkeypatch, tmp_path):
    ...
    assert err2 is not None and "TWGGA_INPUT_ROOT" in err2
...
def test_input_root_default_unrestricted():
    ...
    assert err is not None and "TWGGA_INPUT_ROOT" not in err
