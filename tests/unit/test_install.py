...
def test_installer_ignores_legacy_grok_environment(monkeypatch, tmp_path):
    monkeypatch.setenv("TWGGA_API_KEY", "sk-image2-test")
    monkeypatch.setenv("TWGGA_GROK_API_KEY", "sk-grok-test")
    monkeypatch.setenv("TWGGA_SAVE_DIR", str(tmp_path))
    ...
    assert env["TWGGA_API_KEY"] == "sk-image2-test"
    assert "TWGGA_GROK_API_KEY" not in env
...
def test_phase_a_runtime_is_python_by_default_and_rust_only_when_explicit(tmp_path):
    ...
    binary = tmp_path / ("twgga-image-mcp.exe" if install.sys.platform == "win32" else "twgga-image-mcp")
...
def test_phase_a_writers_preserve_other_mcp_servers(monkeypatch, tmp_path):
    ...
    install.write_claude("/bin/twgga", [], {"TWGGA_API_KEY": "secret"})
    merged = json.loads(claude.read_text(encoding="utf-8"))
    assert merged["theme"] == "dark"
    assert merged["mcpServers"]["other"]["command"] == "other"
    assert merged["mcpServers"]["twgga-image"]["command"] == "/bin/twgga"
    assert merged["mcpServers"]["twgga-image"]["args"] == []
