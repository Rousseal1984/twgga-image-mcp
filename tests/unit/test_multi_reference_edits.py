...
旧主路径 generations + image_urls 被TWGGA静默忽略（image_tokens=0，参考图不起作用）。
...
from twgga_image_mcp import config, http_client
from twgga_image_mcp.http_client import Endpoint, _call_endpoint
import server
...
def test_call_endpoint_expands_image_array(monkeypatch):
    ...
    ep = Endpoint(
        url="https://twgga.work/v1/images/edits",
        multipart={
            "model": "gpt-image-2",
            "prompt": "x",
            "size": "1024x1024",
