...
@pytest.mark.skipif(
    ...
    reason="paid live SDK test requires TWGGA_RUN_LIVE_TESTS=1",
)
def test_one_real_rust_sdk_generation_without_persisting_or_logging_key() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="twgga-rust-live-sdk-") as temp_dir:
        ...
        report_path = os.environ.get("TWGGA_LIVE_REPORT")
        if report_path:
            sanitized = {
                "ok": result["ok"],
                "model": result["model"],
