...
from tests.contract.mock_twgga_api import MockTwggaApi, png_bytes
...
def protocol_metrics(command: list[str], repeats: int) -> dict[str, Any]:
    ...
    for _ in range(repeats):
        with tempfile.TemporaryDirectory(prefix="twgga-benchmark-protocol-") as temp_dir:
...
def heavy_metric(command: list[str], scenario: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"twgga-benchmark-{scenario}-") as temp_dir:
        ...
        input_root.mkdir()
        home_root.mkdir()
        with MockTwggaApi() as mock:
            overrides = mock.env()
            overrides.update(
                {
                    "HOME": str(home_root),
                    "USERPROFILE": str(home_root),
                    "TWGGA_INPUT_ROOT": str(input_root),
                }
            )
            if scenario == "large_url":
                overrides["TWGGA_RESPONSE_FORMAT"] = "url"
                arguments = {
                ...
                overrides["TWGGA_RESPONSE_FORMAT"] = "b64_json"
                arguments = {
    ...
    with contextlib.ExitStack() as stack:
        ...
        for index in range(count):
            temp_dir = stack.enter_context(
                tempfile.TemporaryDirectory(prefix=f"twgga-benchmark-multiprocess-{index}-")
            )
            root = Path(temp_dir).resolve()
            roots.append(root)
...
def main() -> None:
    ...
    parser.add_argument(
        "--rust-command",
        default=str(REPO_ROOT / "target" / "release" / "twgga-image-mcp"),
    )
    parser.add_argument("--repeat", type=int, default=3)
    parser.add_argument("--process-count", type=int, default=4)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if os.environ.get("TWGGA_RUN_LIVE_TESTS") == "1":
        raise SystemExit("benchmark refuses TWGGA_RUN_LIVE_TESTS=1; it is strictly offline")
