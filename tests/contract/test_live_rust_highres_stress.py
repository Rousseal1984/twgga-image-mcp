...
@pytest.mark.skipif(
    not (
        ...
        and os.environ.get("TWGGA_RUN_LIVE_HIGHRES_STRESS") == "1"
    ),
    reason=(
        "paid 2K/4K stress requires TWGGA_RUN_LIVE_TESTS=1, "
        "TWGGA_RUN_LIVE_STRESS=1 and TWGGA_RUN_LIVE_HIGHRES_STRESS=1"
    ),
)
def test_real_rust_2k_4k_cross_process_pressure_matrix() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="twgga-rust-live-highres-") as temp_dir:
            ...
            env.update(
                {
                    ...
                    "TWGGA_SAVE_DIR": str(output_root),
                    "TWGGA_SAVE_DIR_ROOT": str(output_root),
                    "TWGGA_MODEL": "gpt-image-2",
                    "TWGGA_USE_SHELL_PROXY": "0",
        ...
        report = {
            ...
            "provider_base_url": "https://twgga.work",
        ...
        report_path = os.environ.get("TWGGA_LIVE_HIGHRES_STRESS_REPORT")
        if report_path:
