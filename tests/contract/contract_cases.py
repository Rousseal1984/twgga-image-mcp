...
from tests.contract.mock_twgga_api import MockTwggaApi, png_bytes, rgba_png_bytes
...
CASES: tuple[CaseSpec, ...] = (
    ...
    CaseSpec(
        ...
        env=(("TWGGA_CONTRACT_TESTING", "1"), ("TWGGA_TEST_API_TIMEOUT_MS", "50")),
    ),
    CaseSpec(
...
def run_case(command: list[str], name: str) -> dict[str, Any]:
    spec = _spec(name)
    with tempfile.TemporaryDirectory(prefix=f"twgga-contract-{name}-") as temp_dir:
