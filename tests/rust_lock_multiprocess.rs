...
#[test]
fn lock_probe_child() {
    if std::env::var("TWGGA_LOCK_CHILD").ok().as_deref() != Some("1") {
        return;
    }
    let path = std::env::var_os("TWGGA_LOCK_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|| panic!("TWGGA_LOCK_PATH is required"));
...
fn separate_processes_serialize_on_the_same_lock_file() {
    ...
    let mut first = Command::new(&executable)
        ...
        .env("TWGGA_LOCK_CHILD", "1")
        .env("TWGGA_LOCK_PATH", &lock_path)
        .spawn()
        .unwrap_or_else(|error| panic!("{error}"));
