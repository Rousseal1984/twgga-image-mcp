...
def ensure_save_dir(label: str) -> Path:
    """统一把测试图扔到 /tmp/twgga-bench-<label>，避免污染用户 ~/Pictures。"""
    root = Path(os.environ.get("TWGGA_BENCH_DIR", "/tmp/twgga-bench")) / label
    root.mkdir(parents=True, exist_ok=True)
    # server.py 启动时把 TWGGA_SAVE_DIR_ROOT 当沙箱根，必须在 import server 之前设
    os.environ["TWGGA_SAVE_DIR_ROOT"] = str(root)
    os.environ["TWGGA_SAVE_DIR"] = str(root)
    return root
...
def has_image2_key() -> bool:
    return bool(os.environ.get("TWGGA_API_KEY", "").strip())


def live_tests_enabled() -> bool:
    """Paid/live benchmark calls require an explicit, exact opt-in."""
    return os.environ.get("TWGGA_RUN_LIVE_TESTS", "").strip() == "1"


def has_grok_key() -> bool:
    return bool(
        os.environ.get("TWGGA_GROK_API_KEY", "").strip()
