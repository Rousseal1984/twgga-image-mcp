...
_SANDBOX = Path(tempfile.gettempdir()) / "twgga-smoke"
_SANDBOX.mkdir(parents=True, exist_ok=True)
os.environ["TWGGA_SAVE_DIR_ROOT"] = str(_SANDBOX)
os.environ["TWGGA_SAVE_DIR"] = str(_SANDBOX)

# 强制移除继承的 key，保证 smoke 永远离线并触发“未配置 API key”错误链路。
os.environ.pop("TWGGA_API_KEY", None)
...
async def test_proto(server_command: list[str]) -> None:
    ...
    r = await recv()
    if r.get("result", {}).get("serverInfo", {}).get("name") == "twgga-image":
    ...
    if content and content[0].get("type") == "text":
        ...
        if info_data.get("base_url") == "https://twgga.work":
...
async def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "例如 --server-command './target/debug/twgga-image-mcp serve'"
        ),
    )
    args = ap.parse_args()
