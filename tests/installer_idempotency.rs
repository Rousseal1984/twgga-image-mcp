...
fn stable_binary_install_is_atomic_executable_and_idempotent() {
    ...
    let destination = temp
        .path()
        .join("data-local/twgga-image-mcp/bin")
        .join(if cfg!(windows) {
            "twgga-image-mcp.exe"
        } else {
            "twgga-image-mcp"
...
fn verified_client_config_writes_backup_reparse_and_preserve_original_on_failure() {
    ...
    let launch = ClientLaunchSpec::new(
        Path::new("/stable/Twgga MCP/twgga-image-mcp").to_path_buf(),
        Vec::new(),
        BTreeMap::from([("TWGGA_SAVE_DIR".into(), "/tmp/图片".into())]),
    );
...
fn invalid_existing_toml_is_never_overwritten_or_backed_up_as_success() {
    ...
    let launch = ClientLaunchSpec::new(
        Path::new("/stable/twgga-image-mcp").to_path_buf(),
        Vec::new(),
        BTreeMap::new(),
    );
