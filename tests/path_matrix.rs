...
use twgga_image_mcp::config::{AppPaths, EnvironmentSnapshot, PathPolicy, PathSource};
...
fn app_paths_resolve_once_from_injected_sources_without_ambient_home_or_cwd() {
    ...
    assert_eq!(
        paths.save_root,
        fs::canonicalize(source.home.join("Pictures/twgga-out"))
            .unwrap_or_else(|error| panic!("{error}"))
    );
    assert_eq!(paths.default_save_dir, paths.save_root);
    assert_eq!(paths.input_root, None);
    assert_eq!(paths.cache_dir, paths.home.join(".cache/twgga-image"));
...
fn save_root_is_home_based_and_save_dir_is_root_based_never_cwd_based() {
    ...
    let paths = AppPaths::resolve(
        &environment([
            ("TWGGA_SAVE_DIR_ROOT", "relative-root".into()),
            ("TWGGA_SAVE_DIR", "nested/output".into()),
        ]),
        source.clone(),
    )
    .unwrap_or_else(|error| panic!("{error}"));
...
fn tilde_expansion_is_exact_and_rejects_named_user_forms() {
    ...
    let source = source(&temp);
    let paths = AppPaths::resolve(
        &environment([("TWGGA_SAVE_DIR_ROOT", "~/safe".into())]),
        source.clone(),
    )
    .unwrap_or_else(|error| panic!("{error}"));
    assert_eq!(
    ...
    let error = AppPaths::resolve(
        &environment([("TWGGA_SAVE_DIR_ROOT", "~someone/output".into())]),
        source,
    )
...
fn windows_root_checks_are_component_aware_and_case_insensitive() {
    ...
    let paths = AppPaths::resolve(
        &environment([("TWGGA_SAVE_DIR_ROOT", path_text(&root))]),
        source,
    )
    .unwrap_or_else(|error| panic!("{error}"));
    let policy = PathPolicy::new(&paths);
