...
sys.path.insert(0, os.environ["TWGGA_REPO_ROOT"])
import server
def _first_error(r):
    if r.get("error"):
        return str(r["error"])
...
async def main():
    t0 = time.perf_counter()
    r = await server.image_generate(
        prompt=os.environ["TWGGA_PROMPT"],
        size=os.environ["TWGGA_SIZE"],
        n=1,
        model=os.environ["TWGGA_MODEL"],
...
def _spawn_worker(idx: int, *, model: str, size: str, save_root: Path) -> subprocess.Popen:
    env = os.environ.copy()
    env["TWGGA_REPO_ROOT"] = str(Path(__file__).resolve().parent.parent)
    env["TWGGA_PROMPT"] = PROMPT
    env["TWGGA_SIZE"] = size
    env["TWGGA_MODEL"] = model
    env["TWGGA_SAVE_DIR_ROOT"] = str(save_root)
    env["TWGGA_SAVE_DIR"] = str(save_root)
...
def main() -> None:
    ...
    if not live_tests_enabled():
        print("[ERR] 真实压力测试默认关闭；仅 TWGGA_RUN_LIVE_TESTS=1 时允许发出付费请求")
        sys.exit(1)
    if not has_image2_key():
        print("[ERR] TWGGA_API_KEY 未配置")
        sys.exit(1)
