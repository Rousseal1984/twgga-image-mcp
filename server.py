"""TWGGA gpt-image-2 MCP server (entry point).

历史上本文件是单文件 ~2500 行。重构后所有辅助函数 / 配置常量 / HTTP / 锁 / 保存 /
extract / routing / validation 均迁移到 twgga_image_mcp/ 内部 package。

本文件只保留 3 件事：
  1. import 必要符号到 server module 命名空间（外部 tests / tools 用 `import server`）
  2. mcp = FastMCP("twgga-image") + 5 个 @mcp.tool() 函数（带完整 docstring 供 LLM schema）
  3. main() entry
...
def _load_frozen_tool_descriptions() -> dict[str, str]:
    contract_path = Path(__file__).resolve().parent / "twgga_image_mcp" / "tool_contract.json"
...
from twgga_image_mcp import __version__
from twgga_image_mcp.config import (
...
from twgga_image_mcp.sizes import (
...
from twgga_image_mcp.routing import (
...
from twgga_image_mcp.io_safety import (
...
from twgga_image_mcp.locks import (
...
from twgga_image_mcp.save import (
    ImageSaveError,
...
def _get_key(override: str | None) -> str:
    ...
    if not key:
        raise RuntimeError(
            "未配置 API key。请设置 TWGGA_API_KEY 环境变量，或在调用时传 api_key 参数。"
        )
    return key
...
def _get_grok_key(override: str | None) -> str:
    ...
    if not key:
        raise RuntimeError(
            "未配置TWGGA Grok API key。请设置 TWGGA_GROK_API_KEY 环境变量，或在调用时传 api_key 参数。"
        )
    return key
...
async def image_generate(
    ...
    """文本生成图像（text-to-image）。TWGGA代理 + gpt-image-2 系列。
    ...
    [SIZE 选取建议]
      ...
      - 用户提到"高清/4K/海报/壁纸" → "3840x2160"（横）或 "2160x3840"（竖）。
    ...
    Common errors and what to do:
        ...
        "未配置 API key" → 设置 TWGGA_API_KEY 环境变量或传 api_key 参数。
    """
    ...
    if use_grok:
        ...
        if size_mode == "backend":
            notes.append("Grok 尺寸模式 TWGGA_GROK_SIZE_MODE=backend：保留后端原始像素。")
        else:
            notes.append(
                f"Grok 后端只按 resolution/aspect_ratio 生成；保存前会按 TWGGA_GROK_SIZE_MODE={size_mode} "
    ...
    saved: list[dict] = []
    errors: list[str] = []
    # 客户端循环 N 次单图请求（TWGGA image_generation tool 不接受 n 字段）。
...
async def image_edit(
    ...
    [尺寸能力]（2026-08-14 当前线路实测）
      - 1K：1024×1024 edits 成功；实际像素以 saved.actual_size 为准。
      - 2K：2048×1152 edits 成功；实际像素以 saved.actual_size 为准。
      - 4K：3840×2160 edits 成功；实际像素以 saved.actual_size 为准。
    ...
    if _is_grok_model(eff_model):
        ...
        notes.append(
            ...
            "size 只映射为 resolution/aspect_ratio，实际像素由 Grok/TWGGA 返回决定。"
        )
        if size_mode == "backend":
            notes.append("Grok 尺寸模式 TWGGA_GROK_SIZE_MODE=backend：保留后端原始像素。")
        else:
            notes.append(
                f"Grok 保存前会按 TWGGA_GROK_SIZE_MODE={size_mode} 本地归一化到请求 size={size}。"
            )
        if mask_path:
...
async def image_batch_edit(
    ...
    eff_model, notes = _resolve_model(model, size)
    if _is_grok_model(eff_model):
        msg = "Grok 模型当前不支持 image_batch_edit 批量逐张编辑；请改用 image_edit 单图、image_multi_reference 多图参考，或改用 gpt-image-2。"
        return {"ok": False, "error": msg, "errors": [msg], "total": len(image_paths)}
    is_quality = _is_quality_model(eff_model)
    ...
    concurrency = 1 if _size_tier(size) in ("2k", "4k") else 5
...
async def image_multi_reference(
    ...
    [LIMITS]（当前真实状态，会变化）
      ...
      - 2K/4K：走 edits/image[]；无本地尺寸硬阻断。
        高分辨率多图融合的耗时会随参考图数量增加，成功后以 saved.actual_size / size_honored 核对真实像素。
      - TWGGA多图间歇拒绝时会按重试策略处理，仍失败则直接返回 Images API 错误。
      - 单张参考图建议 ≤2MB；总输入 ≤8MB（TWGGA代理上限实测约 10MB）。
...
def server_info() -> dict[str, Any]:
    ...
    return {
        ...
        "safety_constraints": {
            ...
            "save_dir_root": (
                f"所有输出强制落在 TWGGA_SAVE_DIR_ROOT={_SAVE_ROOT} 之下；"
                "传 root 之外路径会被拒"
            ),
            ...
            "base_url_locked": "base_url 锁在启动期 TWGGA_BASEURL env，运行期 tool 不接受参数（防 key 外泄到攻击者 host）",
        },
        "recommended_sizes": {
        ...
        "capability_matrix": {
            "image_generate": {
                ...
                "2k_quality": "可用；N 强制为 1；实际像素以 saved.actual_size 为准",
                "4k_quality": "可用；N 强制为 1；实际像素以 saved.actual_size 为准",
            },
            "grok_image_generate": {
                "1k": "暂时关闭，待服务器支持后再启用",
                "2k": "暂时关闭，待服务器支持后再启用",
            ...
            "image_multi_reference": {
                ...
                "2k_quality": "可用；失败时直接返回 Images API 错误",
                "4k": "可用：走 edits + image[]；核对 saved.actual_size",
            },
        },
        "retry_policy": {
        ...
        "response_handling": {
            "saved_to_disk": "所有生成的图片落盘到 save_dir（默认 cwd/out 或 TWGGA_SAVE_DIR）",
