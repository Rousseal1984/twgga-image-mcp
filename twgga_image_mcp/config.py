...
# ---------- 配置（env 可覆盖）----------
DEFAULT_BASEURL = os.environ.get("TWGGA_BASEURL", "https://twgga.work")
API_KEY = os.environ.get("TWGGA_API_KEY", "")
DEFAULT_MODEL = os.environ.get("TWGGA_MODEL", "gpt-image-2")
GROK_BASEURL = os.environ.get("TWGGA_GROK_BASEURL", DEFAULT_BASEURL)
...
GROK_SIZE_MODE = os.environ.get("TWGGA_GROK_SIZE_MODE", "contain").strip().lower()
# TWGGA是国内站，不应走 shell 的 SOCKS/HTTP 代理；默认 trust_env=False。
# 设 TWGGA_USE_SHELL_PROXY=1 才让 httpx 拾取 HTTPS_PROXY/HTTP_PROXY/ALL_PROXY。
_TRUST_ENV = os.environ.get("TWGGA_USE_SHELL_PROXY", "").strip() in ("1", "true", "yes")

# API 响应格式策略：
#   auto（默认）— 先 url 请求并下载落盘，失败再重试 API response_format=b64_json
#   url — 仅 url； b64_json — 仅 b64_json
_rf_raw = os.environ.get("TWGGA_RESPONSE_FORMAT", "auto").strip().lower()
if _rf_raw == "url":
...
_trusted_hosts_raw = os.environ.get("TWGGA_TRUSTED_DOWNLOAD_HOSTS", "oss.filenest.top").strip()
...
SUPPORTED_IMAGE_MODELS = (STANDARD_MODEL,)
...
# 仅离线 contract harness 可缩短超时；生产环境忽略 TWGGA_TEST_API_TIMEOUT_MS。
_CONTRACT_TESTING = os.environ.get("TWGGA_CONTRACT_TESTING", "").strip() == "1"
try:
    _contract_timeout_ms = float(os.environ.get("TWGGA_TEST_API_TIMEOUT_MS", "600000"))
except ValueError:
