...
def _resolve_save_dir(save_dir: str | None) -> tuple[Path | None, str | None]:
    ...
    try:
        ...
        return None, (
            ...
            f"留空让 MCP 用默认目录，或先把 TWGGA_SAVE_DIR_ROOT 改到你想要的位置。"
        )
...
def _validate_image_path(image_path: str, label: str = "image_path") -> tuple[Path, bytes, str, str | None]:
    ...
    # 可选输入白名单根（TWGGA_INPUT_ROOT）：启用后拒绝根外路径，resolve 后比对可挡符号链接逃逸。
    ...
    if _INPUT_ROOT is not None:
        try:
            ...
            return p, b"", "", (
                f"{label} 必须在 TWGGA_INPUT_ROOT={_INPUT_ROOT} 之下（已启用输入路径白名单）；收到 {image_path!r}"
            )
    if not p.is_file():
