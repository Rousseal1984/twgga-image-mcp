...
`tests/contract/fixtures/python/` 的 STDIO 快照和本地 mock Twgga API 固定，再通过同一组黑盒
...
一个进程只构造一次不可变 `Config`。它读取并规范化所有 `TWGGA_*` 环境变量，持有：
...
进程级共享 `reqwest::Client` 只负责 Twgga API；连接池和 keep-alive 由该 client 复用。
默认显式禁用环境代理。只有 `TWGGA_USE_SHELL_PROXY=1` 时，启动期读取代理环境并构造代理。
