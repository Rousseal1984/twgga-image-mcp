...
更新时间：2026-08-23  
Python reference：仓库 HEAD `578b32a` 上保留的 `server.py` / `twgga_image_mcp/`  
Rust：`twgga-image-mcp 0.3.0` + 官方 `rmcp 3.1.4`
...
| TWGGA_INPUT_ROOT / symlink | Exact + stronger implementation | 公共拒绝文本相同；Rust 上传已验证磁盘快照，不重新按不可信路径读取 |
...
| 默认不读 shell proxy | Exact | 仅 `TWGGA_USE_SHELL_PROXY=1` 读取 HTTP(S)/ALL/NO_PROXY |
