...
TWGGA_RUN_LIVE_TESTS=0 .venv/bin/python -m tests.contract.benchmark_runtime \
  --output /tmp/twgga-rust-benchmark.json
```

脚本拒绝在 `TWGGA_RUN_LIVE_TESTS=1` 时运行。全部调用都指向同进程启动的
`127.0.0.1` mock Twgga API/forward proxy；没有真实生图请求，也没有把远端渲染延迟计入结果。
