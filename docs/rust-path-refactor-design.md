...
| Save root | `TWGGA_SAVE_DIR_ROOT`, otherwise `home/Pictures/twgga-out` | relative values currently use process cwd | no | yes | itself after create + canonicalize | `Config`, `Storage` |
| Default save directory | `TWGGA_SAVE_DIR`, otherwise save root | relative values currently use process cwd | no | yes | save root | `Storage::resolve_save_dir` |
| Tool `save_dir` | MCP UTF-8 string | currently process cwd | no | yes | save root | all four image tools through `Storage` |
| Input root | optional `TWGGA_INPUT_ROOT` | relative values currently use process cwd | yes when configured | no | itself after canonicalize | image validation |
...
| Lock file | hard-coded `home/.cache/twgga-image/bigsize.lock` | n/a | no | yes | parent cache directory | `HttpExecutor` / `BigRequestGate` |
| Output temp | generated `.twgga-*.tmp` | resolved output directory | no | yes | capability-open save root | `Storage` |
...
- tool test fixtures independently rebuild environment path maps and frequently call
  `to_string_lossy()`.
- the Python reference uses `Path.home()/.cache/twgga-image/bigsize.lock`; Rust currently uses the
...
9. **Plaintext client secret:** the Rust installer currently copies `TWGGA_API_KEY` into client
...
- `TWGGA_SAVE_DIR_ROOT`: exact `~`, `~/...`, and `~\...` expand against captured home; an absolute
...
- `TWGGA_SAVE_DIR`: absolute values must be below canonical save root; relative values resolve below
  save root.
- tool `save_dir`: same rule as `TWGGA_SAVE_DIR`.
- `TWGGA_INPUT_ROOT`: root syntax follows the save-root rule and is canonicalized at startup.
...
- lock default remains `home/.cache/twgga-image/bigsize.lock` in both Python and Rust so mixed
