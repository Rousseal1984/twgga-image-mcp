...
- `TWGGA_API_KEY`、legacy Grok key 和 tool-level key override 进入 `secrecy::SecretString`；
  `Debug` 固定显示 `[REDACTED]`。
...
启用 `TWGGA_INPUT_ROOT` 时，路径先 canonicalize，再相对于 canonical root 检查，并通过
...
- `TWGGA_SAVE_DIR_ROOT` 创建后 canonicalize，并以 `cap-std::fs::Dir` 作为 capability root。
...
- 配置文件使用同目录临时文件、`sync_all`；先以真实 TOML/JSON parser 验证临时文件和 PathBuf
  round-trip，再生成 0600 backup 并 atomic replace。Claude JSON/Codex TOML 只修改 twgga-image
  节，不持久化 API key。
...
- `198.18.0.0/15` 只有 `TWGGA_ALLOW_FAKE_IP_DOWNLOAD=1` 且 hostname 精确/子域匹配 trusted
...
只有 `TWGGA_USE_SHELL_PROXY=1` 才启用代理。部署方若需要端到端 DNS pinning，应禁用 shell
proxy 或使用可信、可审计的代理。

## HTTP、内存与重试

- Twgga API 共享一个进程级 reqwest client；默认无环境代理，redirect disabled。
